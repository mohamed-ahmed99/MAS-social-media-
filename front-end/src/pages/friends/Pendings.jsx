...
export default function FriendRequests() {
    ...
    const endPoint = `/api/relationship/from-me?limit=${query.limit}&page=${query.page}&type=friend&status=pending`
        ...
        <div className='space-y-4 pb-4'>
              ...
              <div className='grid grid-cols-3 lmd:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-6 gap-2 lmd:gap-4 px-3 lmd:pr-5 lmd:pl-1 '>
                  {allUsers.map((user, index) => {
                    ...
                    return <FriendCards key={index} userData={user} blackBtn="CANCEL_REQUEST" grayBtn=""/>
                  })} 
              </div>
              ...
              <div className=' w-full '>
                  ...
                  {!loading_g && data_g?.users?.length >= query.limit && (
                      <SeeMoreBtn setQuery={setQuery} loading={loading_g} />
                  )}
                  
                  {/* see more button for loading */}
                  {loading_g && query.page > 1 && (
                      <SeeMoreBtn setQuery={setQuery} loading={loading_g} />
                  )}
              </div>
            ...
            {allUsers.length == 0  &&
              <div className='flex items-center justify-center h-[calc(100vh-30vh)] '>
                <UI_Message 
                    ...
                    text={`You didn't send any requests yet.`}  
                />
              </div>
            }
    </div>
